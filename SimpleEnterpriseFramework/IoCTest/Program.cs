...
namespace IoCTest
{
    class Program
    {
        ...
        private async static void DemoDatabase()
        {
            ...
            //User user = User.getInstance("username2", "123", "email", "phone", "address", "role");
            ...
            //MemberShip.MemberShip.createRoleAsync(Role.getInstance("employee1"));
            //MemberShip.MemberShip.createRoleAsync(Role.getInstance("employee2"));
            //Role[] roles = new Role[2];
            //Role r = await MemberShip.MemberShip.findRoleByNameAsync("employee1");

            //roles[0] = r;
            //r = await MemberShip.MemberShip.findRoleByNameAsync("employee2");
            //roles[1] = r;

            //MemberShip.MemberShip.AddRolesToUserAsync(roles,"SMQRSFV");
            ////User[] users =await MemberShip.MemberShip.findUserByRoleAsync("employee2"); //--- OK
            //string[] users = new string[2];

            // User user =await MemberShip.MemberShip.findUserByIdAsync("SMQRSFV");
            // User test =await MemberShip.MemberShip.findUserByIdAsync("MNLNFOR");
            //users[0] = user.Id;
            //users[1] = test.Id;
            //MemberShip.MemberShip.AddRoleToUsersAsync(r, users);//--OK
            //if(users != null)
            //{
            ...
            //DataTable result1 = await db.GetTable("accounts", str0, str1);
            //Console.WriteLine(result1.Rows.Count);
